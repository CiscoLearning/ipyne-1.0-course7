...
# TASK 4 ADDITION - Advanced Logging Setup
handler = TimedRotatingFileHandler('backup.log', when='M', interval=1, backupCount=12)
...
def commit_backup(backup_dir, commit_message):
    ...
    log_and_print("Backup committed to Git repository.")
...
def fetch_running_config(testbed, device_name):
    ...
    try:
        ...
        log_and_print(f"Error on {device_name}: {e}", "error")
        return None
...
def backup_config(testbed, backup_dir):
    ...
    for device_name in testbed.devices:
        log_and_print(f"Backing up {device_name}...")
        ...
        if config:
            ...
            log_and_print(f"Skipped {device_name}")
...
def compare_configs(backup_dir, device_name):
    ...
    if not latest_backup or not previous_backup:
        log_and_print(f"Not enough backups to compare for {device_name}")
        return
    ...
    with open(previous_backup, "r") as old_file, open(latest_backup, "r") as new_file:
        ...
        if diff.diffs:
            log_and_print(f"Differences found for {device_name}:")
            log_and_print(str(diff.diffs))
        else:
            log_and_print(f"No configuration changes detected for {device_name}")
